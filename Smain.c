...
#define SPDF_PORT 8093
#define STEXT_PORT 8092
...
void handle_dtar(int client_sock, char *filetype) {
    ...
    if (strcmp(filetype, ".c") == 0) {
        ...
        //tar_file = fopen("pdffiles.tar", "wb");

        // Receive the tar file from Spdf server
        // while ((file_size = recv(spdf_sock, buffer, BUFFER_SIZE, 0)) > 0) {
        //     fwrite(buffer, 1, file_size, tar_file);
        //     if (file_size < sizeof(buffer))
        //     {
        //         printf("End of file detected\n");
        //         break; // End of file
        ...
        // Open the tar file to send it to the client
